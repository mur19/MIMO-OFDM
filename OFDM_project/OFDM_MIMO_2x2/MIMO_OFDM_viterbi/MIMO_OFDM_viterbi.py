        ...
        if mmse_solver == 'classic':
            ...
            for i in range(self.OFDM_symbols):
                for j in range(self.subcarriers):
                    ...
                    H_house = np.vstack([H_mat, I/self.SNR])
                    ...
                    for k in range(2*k_qam):
                        ...
                        for q in range(self.M**2):
                            ...
                            if bits_for_llr_int[q][k] == 0:
                                ...
                                llr_0 = np.append(llr_0, np.exp(-np.sum(np.abs(equalized_signal[i, j, :] - H_mat @ qam_grid[q])**2) / sigma2))
                            
                            elif bits_for_llr_int[q][k] == 1:
                                
                                llr_1 = np.append(llr_1, np.exp(-np.sum(np.abs(equalized_signal[i, j, :] - H_mat @ qam_grid[q])**2) / sigma2))
